...
from utilities import adversarial_dropout, batch_noise
...
# Virtual adversarial dropout parameters
flags.DEFINE_float('adv_drop_change_rate', 0.05, 'adv_drop_change_rate')
flags.DEFINE_integer('adv_drop_change_iteration', 2, 'adv_drop_iteration')
...
def virtual_adversarial_loss(logits, embedded, mask, inputs,
  ...
  for _ in xrange(FLAGS.num_power_iteration):
    ...
    d_logits = logits_from_embedding_fn(embedded + d, mask)
  ...
  vadv_logits = logits_from_embedding_fn(embedded + perturb, mask)
...
def adversarial_dropout_loss(logits, embedded, mask, inputs,
  ...
  # Initialize perturbation with random noise.
  ini_mask = tf.ones_like(mask)
  change_limit = int(FLAGS.adv_drop_change_rate*FLAGS.rnn_cell_size)
  ...
  d_logits = logits_from_embedding_fn(embedded, ini_mask)
  kl = _kl_divergence_with_logits(logits, d_logits, weights)
  Jocob, = tf.gradients(
        kl,
        ini_mask,
        aggregation_method=tf.AggregationMethod.EXPERIMENTAL_ACCUMULATE_N)
  Jocob = tf.stop_gradient(Jocob)
  tf.summary.scalar('influence_map', tf.reduce_mean(Jocob))
  
  adv_mask = adversarial_dropout(ini_mask, Jocob, change_limit)
  
  adt_logits = logits_from_embedding_fn(embedded, adv_mask)
  return _kl_divergence_with_logits(logits, adt_logits, weights)
...
def iterative_adversarial_dropout_loss(logits, embedded, mask, inputs,
  ...
  # Initialize perturbation with random noise.
  adv_mask = tf.ones_like(mask)
  change_limit = int(FLAGS.adv_drop_change_rate*FLAGS.rnn_cell_size)
  step_change_limit = int(change_limit/FLAGS.adv_drop_change_iteration)
  # Perform finite difference method and power iteration.
  # See Eq.(8) in the paper http://arxiv.org/pdf/1507.00677.pdf,
  # Adding small noise to input and taking gradient with respect to the noise
  # corresponds to 1 power iteration.
  for k in range(0, FLAGS.adv_drop_change_iteration):
      d_logits = logits_from_embedding_fn(embedded, adv_mask)
      kl = _kl_divergence_with_logits(logits, d_logits, weights)
      Jocob, = tf.gradients(
            kl,
            adv_mask,
            aggregation_method=tf.AggregationMethod.EXPERIMENTAL_ACCUMULATE_N)
      Jocob = tf.stop_gradient(Jocob)
      Jocob = Jocob*adv_mask
      tf.summary.scalar('influence_map', tf.reduce_mean(Jocob))
      
      adv_mask = adversarial_dropout(adv_mask, Jocob, step_change_limit)
      
  adt_logits = logits_from_embedding_fn(embedded, adv_mask)    
  return _kl_divergence_with_logits(logits, adt_logits, weights)

def fraternal_dropout_loss(logits, embedded, mask, inputs,
                             logits_from_embedding_fn):
  """Virtual adversarial loss.
  Returns:
    kl: float scalar.
  ...
  # Initialize perturbation with random noise.
  mask2 = batch_noise([FLAGS.batch_size, FLAGS.rnn_cell_size], inner_seed=456, keep_prob=FLAGS.keep_prob_lstm_hh)
  vadv_logits = logits_from_embedding_fn(embedded, mask2)
  return _kl_divergence_with_logits(logits, vadv_logits, weights)
