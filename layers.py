...
from tf_lstm import DropoutMaskWrapper
K = tf.keras
  ...
  def build(self, input_shape):
    with tf.device('/cpu:0'):
      self.var = self.add_weight(
          ...
          initializer=tf.random_uniform_initializer(-1., 1.), name='embedding')
...
class dropout_LSTM(object):
  ...
  def __call__(self, x, initial_state, seq_length, state_mask):
    with tf.variable_scope(self.name, reuse=self.reuse) as vs:
      cell_list = [
          tf.contrib.rnn.BasicLSTMCell(
              self.cell_size,
      ...
      cell_list[-1] = DropoutMaskWrapper(cell_list[-1], state_mask, dtype=tf.float32)
      ...
      # shape(lstm_out) = (batch_size, timesteps, cell_size)
      #lstm_out = tf.reshape(lstm_out, [-1, self.cell_size])
...
def optimize(loss,
  ...
  * Summarizes variables and gradients
  ...
  with tf.name_scope('optimization'):
    ...
    non_embedding_grads_and_vars = [(n_g, n_v) for (n_g, n_v) in zip(ne_grads, ne_vars)] 
